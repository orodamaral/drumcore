...
| `start_autotune` | `pad` (0-31) | Inicia o assistente de auto-calibração (Fase O) pra esse pad — ver [01-decisoes-arquiteturais.md](01-decisoes-arquiteturais.md). `error` (`invalid_pad`) se o pad não existir/não for `primary`, (`channel_disabled`) se o canal estiver desligado (`enabled: false`, Fase N), ou (`not_applicable_for_choke`) se `pad_type` for 9 (`PAD_CHOKE` — gatilho binário, sem envelope de piezo pra calibrar). Resposta: uma série de `autotune_status` conforme o assistente avança (não é só uma resposta única — ver abaixo). |
